<template>
	<view class="ui-form-group" :class="[{ 'first-item': isFirstChild }, { disabled: disabled }, ui]">
		<view class="ui-form-title" :style="[{ width: `${titleWidth + 1.4}em`}]" :class="[{'self-start':titleTop}]" v-if="showTitle">
			<text v-if="icon" class="ui-form-icon" :class="[icon,{'pl-2':title.length==0}]"></text>
			{{ title }}
			<text class="ui-form-required" v-if="required">*</text>
		</view>
		<view class="ui-form-content" :class="[contentui]">
			<slot></slot>
		</view>
		<view class="ui-form-action" v-if="slots.action">
